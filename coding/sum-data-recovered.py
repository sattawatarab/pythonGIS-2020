...
rd = pd.read_csv(outdata+'time_series_19-covid-Recovered.csv')
...
#print(rows[ab:])
corona_th = (rows[1:])
corona_th.to_json(outdata2+"recovered_th.json", orient = "records", date_format = "epoch", double_precision = 10, force_ascii = False, date_unit = "ms", default_handler = None)
corona_th_day = (rows[ab:])
corona_th_day.to_json(outdata2+"recovered_th_day.json", orient = "records", date_format = "epoch", double_precision = 10, force_ascii = False, date_unit = "ms", default_handler = None)

point_sel = rename.iloc[:, list(range(4)) + [-1]]
point_sel2 = point_sel.rename(columns={point_sel.columns[-1]:"total"})
point_sel2.to_csv(outdata2+'all_data_recovered.csv')
#create xy
rd2 = pd.read_csv(outdata2+'all_data_recovered.csv')
geometry = [Point(xy) for xy in zip(rd2['Long'], rd2['Lat'])]
crs = {'init': 'epsg:4326'}
gdf = gpd.GeoDataFrame(rd2, crs=crs, geometry=geometry)
gdf.to_file(outdata2+'corona_recovered.shp' , encoding = 'utf-8')